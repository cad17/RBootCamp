...
#vector of what the 60 indivs look like at this SNP rs218206_G
#works because we have told R there are row names
testSNP=snps["rs218206_G",]

#counts how often we see 0, 1, or 2
#numbers sum up to 60 
table(testSNP)
...
# Try these commands
table(testSNP) #less than 60 total, indiv 31 is NA
testSNP==1
length(testSNP)
is.na(testSNP)
...
#if you print het it gives you NA
het=sum(testSNP==1,na.rm=TRUE)/sum(!is.na(testSNP))  # but this doesn't 
#sum() lets you do na.rm
#divide by sum of not NA valeus
...
#number of alleles is 2*number of SNPs cause diploids
...
#calculating based on each SNP, so use dim 1 (rows)
...
#this is doing it by hand, not using any pkg 
#returns for a given SNP (x)
