...
###matching info in data frame dd with the pruned tree tt
###key b/c tree had 8K sps
###this lists whats in set one but not set two so we can exclude them 
...
###\n is enter play around with this some more
...
xx <- 1
while(xx < 5) {
  xx <- xx+1; #increase xx by 1 every loop
  if (xx == 3) {
    break; }
  }
print(xx)
...
## ------------------------------------------------------------------------
for(ii in 1:6){
  if (ii %% 2) { #1 = T so if remainder is 1 it's seen as true
    cat(ii, " is odd\n")
    }
  else{
    cat(ii, " is even\n") 
